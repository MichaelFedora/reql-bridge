...
class PostgresQueryDatumPartial extends datum_1.AbstractDatumPartial {
    constructor() { super(); }
    _sel(attribute) {
            ...
            if (params) { // reduce parameters
                for (let i = 0; i < q.params.length; i++) {
                    if (params[i].compile)
                        ...
                        params[i] = util_2.safen(await util_1.resolveHValue(params[i]));
                }
            }
            switch (q.cmd) {
                case 'sel':
...
function createQueryDatum() {
    return selectable_1.makeSelector(new PostgresQueryDatumPartial());
}
