...
const reql_sqlite3_1 = require("./reql-sqlite3");
const level = 'trace';
...
(async () => {
    ...
    const logger = log4js_1.getLogger('db2');
    const test = await reql_sqlite3_1.create();
    ...
    logger.debug('types db: ', await test.table('__reql_typemap__').pluck('types').run().then(a => {
        return a.map(b => JSON.parse(b.types));
    }));
    const table = test.table('test-table');
    ...
    logger.info('table.insert(update): ', await table.insert({ key: 'lime', value: { type: 'bar' } }, { conflict: 'update' }).run());
    logger.info('table: ', await table.run());
    logger.info('table.limit(2): ', await table.limit(2).run());
    logger.info('table.pluck("key", "count").filter({ value: { type: bar } }): ', await table.pluck('key', 'count').filter({ value: { type: 'bar' } }).run());
