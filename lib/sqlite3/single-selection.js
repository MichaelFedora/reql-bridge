...
class SQLite3SingleSelectionPartial extends datum_1.AbstractDatumPartial {
    ...
    async run() {
        ...
        if (!this.query || !this.query.length) {
            return this.db.get(`SELECT * FROM [${tableName}] WHERE [${index}]=${util_2.safen(key)}`)
        ...
        switch (cmd) {
            case 'sel':
                query = `SELECT ${params[0]} FROM [${tableName}] WHERE [${index}]=${util_2.safen(key)}`;
                sel = params[0];
                break;
            case 'update':
                let set = '';
                for (const k in params[0])
                    if (params[0][k] != null) {
                        if (!set)
                            set = `[${k}]=${util_2.safen(params[0][k])}`;
                        else
                            set += `, [${k}]=${util_2.safen(params[0][k])}`;
                    }
                query = `UPDATE [${tableName}] SET ${set} WHERE [${index}]=${util_2.safen(key)}`;
                break;
            case 'replace':
                let repKeys = '';
                let repValues = '';
                ...
                query = `REPLACE INTO [${tableName}] (${repKeys}) VALUES (${repValues}) WHERE ${[index]}=${util_2.safen(key)}`;
                break;
            case 'delete':
                query = `DELETE FROM [${tableName}] WHERE ${[index]}=${util_2.safen(key)}`;
                break;
            default:
                query = `SELECT * FROM [${tableName}] WHERE ${[index]}=${util_2.safen(key)}`;
        }
