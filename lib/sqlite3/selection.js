...
exports.createSelection = exports.SQLite3SelectionPartial = void 0;
...
class SQLite3SelectionPartial extends stream_1.SQLite3Stream {
    ...
    async makeSelection() {
        ...
        for (const key of keys) {
            if (!selection) {
                selection = `[${index}]=${util_2.safen(key)}`;
            }
            else {
                selection += `OR [${index}]=${util_2.safen(key)}`;
            }
        }
        return selection;
    }
...
function createSelection(db, tableName, keys, index, types) {
    const instance = new SQLite3SelectionPartial(db, tableName, keys, index, types);
