...
export class LevelSelectionPartial<T = any> extends LevelStream<T> implements SelectionPartial<T> {
  ...
  constructor(db: WrappedLevelDatabase, tableName: Value<string>,
...
export function createSelection<T = any>(db: WrappedLevelDatabase, tableName: Value<string>,
  keys: Value<any[]>, index: Value<string>, types: Value<SchemaEntry[]>): LevelSelection<T> {
  const instance = new LevelSelectionPartial<T>(db, tableName, keys, index, types);
