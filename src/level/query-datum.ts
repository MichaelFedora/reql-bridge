...
class LevelQueryDatumPartial<T = any> extends AbstractDatumPartial<T> implements DatumPartial<T>, SelectableDatum<T> {
  constructor() { super(); }

  _sel<U extends string | number>(attribute: Value<U>):
    U extends keyof T ? LevelQueryDatumPartial<T[U]> : LevelQueryDatumPartial<any> {
...
export function createQueryDatum<T = any>(): Datum<T> {
  return makeSelector<T>(new LevelQueryDatumPartial()) as any;
}
