...
class PostgresQueryDatumPartial<T = any> extends AbstractDatumPartial<T> implements DatumPartial<T>, SelectableDatum<T> {
  ...
  _sel<U extends string | number>(attribute: Value<U>):
    U extends keyof T ? PostgresQueryDatumPartial<T[U]> : PostgresQueryDatumPartial<any> {
...
export function createQueryDatum<T = any>(): PostgresQueryDatum<T> {
  return makeSelector<T>(new PostgresQueryDatumPartial()) as any;
}
