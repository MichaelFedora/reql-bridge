...
async function test(create: () => Promise<Database>, log?: string) {
  ...
  try {
    ...
    const testTbl = db.table<{ key: string; value: { type: string }; count: number; sale: boolean }>('test_table');
    ...
    logger.info('testTbl.filter(doc => doc("value")("type").eq("bar")): ',
      await testTbl.filter(doc => doc('value')('type').eq('bar')).run());
